...
def parse_parks():
    ...
    # We'll use a simple regex to extract id, name, type, activities
    parks = {}
...
def generate_fields(pid, park_info):
    ...
    if 'biking' in acts or 'mtb' in acts or 'road-biking' in acts:
        ...
        biking = biking.replace('False', 'false') # json compatibility
    ...
    if ptype == 'svra':
        ...
        # Generic state park (inland valleys, hills)
...
def apply_updates():
    ...
    for pid in missing_all:
        ...
        # We find the block for pid in details_str
        pattern = r"(\n\s*'" + re.escape(pid) + r"':\s*\{)"
        ...
        if match:
            s_idx = match.end()
            # find end of this park's block by looking for next park or end of details
